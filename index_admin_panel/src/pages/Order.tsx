...
import { getOrdersFromApi, updateOrderStatus, deleteOrderApi } from "../api/userApi"
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (actionType === "accept") {
        ...
        setOrders(prev =>
          prev.map(order =>
            order.id === selectedOrderId ? { ...order, status: "NoActive" } : order
          )
        )
    ...
    <div className="flex flex-col h-full p-6">
      ...
      <div className="flex border-b border-gray-200 mb-6">
        <button
          ...
          className={`px-6 py-3 text-lg font-semibold transition-colors duration-200 ${activeTab === "orders" ? "text-[#009398] border-b-2 border-[#009398]" : "text-gray-500 hover:text-gray-700"
            }`}
        >
          {t("ordersTab")}
        </button>
        <button
          ...
          className={`px-6 py-3 text-lg font-semibold transition-colors duration-200 ${activeTab === "consultations"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t("sizeDepthCm")}</th>
                ...
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    <div className="flex items-center space-x-2">
                      <button
                        className={`hover:text-green-500 ${order.status === "NoActive" ? "text-green-500" : "text-gray-400"
                          }`}
                        onClick={() => {
