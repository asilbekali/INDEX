  ...
  const handleConfirmConsultation = async () => {
    ...
    try {
      if (actionType === "accept") {
        ...
        setConsultation((prev) =>
          prev
            ? { ...prev, data: prev.data.filter((item) => item.id !== selectedConsultationId) }
            : null
        );
    ...
    <div className="flex flex-col h-full p-6">
      ...
      <div className="flex border-b border-gray-200 mb-6">
        <button
          ...
          className={`px-6 py-3 text-lg font-semibold transition-colors duration-200 ${activeTab === "orders"
                ...
                {consultation.data.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 text-right text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        <button
                          className={`hover:text-green-500 ${item.status === "NoActive" ? "text-green-500" : "text-gray-400"
                            }`}
                          onClick={() => {
